...
def compute_gorkov_objective_local_torch(pf, dx, dy, dz):
    ...
    abs_p2 = (pf.real**2 + pf.imag**2)  # |p|^2
    ...
    laplacian_center = lapU[center_idx]
    ...
    metric = (w_lap * laplacian_center
              - alpha * grad_mag_center
              - beta  * p_pen)

    return metric, laplacian_center, grad_mag_center, p_abs_center
...
def objective_fn_torch(ph_tensor, x_idx, y_idx, z_idx):
    ...
    gorkov_metric, _, _, _ = compute_gorkov_objective_local_torch(pf_local, dx, dy, dz)
    return -gorkov_metric  # minimize
...
def get_metrics_for_logging_new_method(ph_tensor, x_idx, y_idx, z_idx):
    with torch.no_grad():
        ...
        metric, lap_c, gradmag_c, p_abs_c = compute_gorkov_objective_local_torch(pf_local, dx, dy, dz)
        loss = -metric
        return float(lap_c), float(gradmag_c), float(p_abs_c), float(loss)
...
class StepRecorder:
    ...
    로깅:
      - loss, Laplacian, |∇U|, |p|
      - delta_tr_radius(스텝 노름), rho_lin
      - gnorm, pnorm, alpha_imp, cos_theta, r_ratio, sTy, cos_phi, gamma_scale
      - (간헐) lambda_min_est, lambda_max_est, kappa_est
      - 전 phase
    """
    def __init__(self, trial_number, trial_start_time, log_list, total_steps, x0_np):
        ...
        # BFGS 상태 추적(우리 쪽)
        self.n = len(x0_np)
        self.H = np.eye(self.n, dtype=np.float64)  # H_0
    ...
    def __call__(self, xk):
        ...
        pure_laplacian, grad_mag, p_abs, loss = get_metrics_for_logging_new_method(
        ...
        row = {
            ...
            'time_s': elapsed_time,
            'loss': curr_loss,
            'laplacian': float(pure_laplacian),
...
def plot_field_slices(yz_abs_np, xz_abs_np, xy_abs_np, trial_no, final_loss):
    ...
    # 공통 컬러 스케일: XY(z=z0) 단면의 max를 공통 vmax로 사용
...
def decision_space_slices(objective_fun, x_star, g_star, p_star, trial_id):
    # u = -grad, v = p_star에서 u-성분 제거 후 정규화
    u = -g_star
...
def configure_method(method_raw: str):
    """
    method_raw: input()으로 받은 원문.
    허용: 'conventional', 'regularized_conventional', 'force-equillibrium', 'hybrid', 'regularized_hybrid'
    """
    ...
    # --- 메서드별 상수 자동 세팅 ---------------------------------------
    # 공통(요청 사양): W_LAPLACIAN=1.0, ALPHA_F=500.0
    W_LAPLACIAN = 1.0
    ALPHA_F     = 500.0
    ...
    else:  # force-equillibrium, hybrid, regularized_hybrid
        BETA_P = 5e-5
        W_LAPLACIAN_XX = W_LAPLACIAN_YY = W_LAPLACIAN_ZZ = 1
    # -------------------------------------------------------------------

    # O = −∇²_w U + α|∇U| + β·Penalty(|p|)
    if method == "conventional":
        ...
        PRESSURE_PENALTY_MODE = 'abs'  # p 항 없음 -> 영향 없음
    elif method == "hybrid":
    ...
    # 출력 경로: BASE_OUTPUT_DIR / METHOD_NAME / RUN_TAG
